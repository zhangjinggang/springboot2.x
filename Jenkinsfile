pipeline {
  ...
  stages {
    stage('Git Parameter') {
      steps {
        git branch: "${params.BRANCH}", url: 'git@gitlab.choicesoft.com.cn:o2o/sysmonitor.git'
      }
    }
    stage('Build Project') {
      steps {
  ...
  environment {
    ...
    JAVA_OPTS = "-server -Xms1000m -Xmx1000m -Xmn700m -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=300m -XX:+PrintGCDateStamps -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:+UseCMSCompactAtFullCollection -XX:+CMSScavengeBeforeRemark"
  }
  parameters {
    ...
    string(name: 'build_idc', defaultValue: 'default', description: '输入IDC，请参考apollo配置默认为default')
